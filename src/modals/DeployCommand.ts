...
export class DeployCommandModal extends Modal {
    ...
    constructor(app: App, plugin: editingToolbarPlugin, command: any) {
      ...
      this.deployOptions = [
        { id: 'following', name: text('Following Style'), enabled: true },
        { id: 'top', name: text('Top Style'), enabled: true },
        { id: 'fixed', name: text('Fixed Style'), enabled: true },
      ];
      if (this.plugin.settings.isLoadOnMobile) {
        this.deployOptions.push({ id: 'mobile', name: text('Mobile Style'), enabled: true });
      }
    }
    ...
    onOpen() {
      ...
      contentEl.createEl('h3', { text: text('Deploy command to configurations') });
    ...
    private deployCommand() {
      ...
      this.plugin.saveSettings().then(() => {
        ...
        if (deployedCount > 0) {
          ...
          message = text('Command deployed to: ') + deployedConfigs;
        
        } else {
          message = text('Command already exists in selected configurations');
        }
